...
def main():
    ...
    params_roll, _ = optimize(savgoldiff, parking_data['roll'].to_numpy().astype(float), dt)
    _, drolldt_hat = savgoldiff(parking_data['roll'].to_numpy().astype(float), dt, **params_roll)

    params_pitch, _ = optimize(savgoldiff, parking_data['pitch'].to_numpy().astype(float), dt)
    _, dpitchdt_hat = savgoldiff(parking_data['pitch'].to_numpy().astype(float), dt, **params_pitch)

    params_yaw, _ = optimize(savgoldiff, parking_data['yaw'].to_numpy().astype(float), dt)
    _, dyawdt_hat = savgoldiff(parking_data['yaw'].to_numpy().astype(float), dt, **params_yaw)
    ...
    df_footforce.to_csv('parking/parking_footforce_data.csv', index=False)
