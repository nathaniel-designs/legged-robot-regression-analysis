...
angular_roll = np.diff(parking_data['roll']) / dt
angular_pitch = np.diff(parking_data['pitch']) / dt
angular_yaw = np.diff(parking_data['yaw']) / dt
...
angular_vmag_data['Environment'] = 'Parking Lot'
angular_vmag_data.to_csv('parking/parking_angular_vmag_data.csv', index=False)
