...
def main():
    ...
    params_vx, val_vx = optimize(savgoldiff, corridor_data['trans_x'].to_numpy().astype(float), dt)
    x_hat, dxdt_hat = savgoldiff(corridor_data['trans_x'].to_numpy().astype(float), dt, **params_vx)

    params_vy, val_vy = optimize(savgoldiff, corridor_data['trans_y'].to_numpy().astype(float), dt)
    y_hat, dydt_hat = savgoldiff(corridor_data['trans_y'].to_numpy().astype(float), dt, **params_vy)

    params_vz, val_vz = optimize(savgoldiff, corridor_data['trans_z'].to_numpy().astype(float), dt)
    z_hat, dzdt_hat = savgoldiff(corridor_data['trans_z'].to_numpy().astype(float), dt, **params_vz)
    ...
    params_roll, val_roll = optimize(savgoldiff, corridor_data['roll'].to_numpy().astype(float), dt)
    roll_hat, drolldt_hat = savgoldiff(corridor_data['roll'].to_numpy().astype(float), dt, **params_roll)

    params_pitch, val_pitch = optimize(savgoldiff, corridor_data['pitch'].to_numpy().astype(float), dt)
    pitch_hat, dpitchdt_hat = savgoldiff(corridor_data['pitch'].to_numpy().astype(float), dt, **params_pitch)

    params_yaw, val_yaw = optimize(savgoldiff, corridor_data['yaw'].to_numpy().astype(float), dt)
    yaw_hat, dyawdt_hat = savgoldiff(corridor_data['yaw'].to_numpy().astype(float), dt, **params_yaw)
    ...
    angular_vel_mag_data['Environment'] = 'Corridor'
    angular_vel_mag_data.to_csv('corridor/corridor_angular_vel_mag_data.csv', index=False)
